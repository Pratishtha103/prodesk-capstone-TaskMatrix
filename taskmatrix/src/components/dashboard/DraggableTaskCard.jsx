...
export default function DraggableTaskCard({ task, onTaskClick }) {
  ...
  return (
    <div
      ...
      className={`bg-white rounded-lg p-4 shadow-sm hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 cursor-grab active:cursor-grabbing touch-none select-none ${styles.border}`}
    >
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-secondary">
        ...
        {task.assigneeName && (
          <div className="flex items-center gap-1.5 max-w-37.5 select-none" title={task.assigneeName}>
            <div className="w-6 h-6 rounded-full bg-indigo-50 flex items-center justify-center text-[10px] font-bold text-indigo-650 shadow-sm border border-secondary shrink-0">
