...
export default function DashboardPage() {
  ...
  async function handleDragEnd(event) {
    ...
    const statusLabel = newStatus === "inprogress" ? "In Progress" : newStatus === "done" ? "Done" : "To Do";
        ...
        <div className="flex-1 overflow-y-auto min-w-0 flex flex-col justify-between h-full bg-background">
          <main className="p-4 sm:p-6 space-y-8">
            ...
            {activeView === "board" && overdueTasks.length > 0 && (
              <div className="flex items-center justify-between p-4 bg-linear-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl shadow-sm text-sm text-amber-900">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                  ...
                  {(searchQuery || priorityFilter !== "All" || assigneeFilter !== "All") && (
                    <button
                      ...
                      className="text-xs text-red-600 hover:text-red-755 font-semibold px-2 py-1 transition-colors text-center sm:text-left cursor-pointer"
                    >
                      Clear Filters
                    </button>
                  )}
            ...
            {loading ? (
              ...
              tasks.length === 0 ? (
                <EmptyState
                  ...
                  description="Get started by creating your first task to organize your project."
                  icon={ClipboardList}
                  ctaText="+ Create Task"
                  onCtaClick={() => {
                    setActiveTask(null);
                  ...
                  showCta={user?.role === "Admin"}
                />
              ) : filteredTasks.length === 0 ? (
                <EmptyState
                  title="No matching tasks"
                  description="Try modifying your search query or filters to find what you are looking for."
                  icon={Search}
